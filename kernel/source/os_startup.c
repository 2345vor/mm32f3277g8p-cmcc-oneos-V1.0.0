...
#else
static OS_TASK_STACK_DEFINE(main_stack, OS_MAIN_TASK_STACK_SIZE);
...
static os_int32_t os_core_init_start(void)
{
    return 0;
}
OS_INIT_EXPORT(os_core_init_start, "1.", "");

static os_int32_t os_postcore_init_start(void)
{
    return 0;
}
...
static void _k_application_init(void)
{
    ...
    ret = os_task_init(&gs_os_main_task,
                       ...
                       _k_main_task_entry, 
                       OS_NULL,
                       OS_TASK_STACK_BEGIN_ADDR(main_stack),
                       OS_TASK_STACK_SIZE(main_stack),
                       0U);
    if (OS_EOK != ret)
    {
