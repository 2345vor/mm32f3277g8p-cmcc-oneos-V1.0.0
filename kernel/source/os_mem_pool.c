...
static os_err_t _k_mp_add_resourcelist(os_mp_t *mp, os_uint16_t object_alloc_type)
{
    os_list_node_t *pos;
    os_mp_t        *item_mp;
    os_err_t        ret;
...
os_err_t os_mp_init(os_mp_t *mp, const char *name, void  *start, os_size_t size, os_size_t blk_size)
{
    ...
    start_addr = (void *)OS_ALIGN_UP((os_uint32_t)start, OS_ALIGN_SIZE);
    size       = OS_ALIGN_DOWN((os_uint32_t)start + size - (os_uint32_t)start_addr, OS_ALIGN_SIZE);
