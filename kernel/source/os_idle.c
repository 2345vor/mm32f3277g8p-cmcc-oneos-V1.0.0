...
static OS_TASK_STACK_DEFINE(idle_stack, OS_IDLE_TASK_STACK_SIZE);
...
void k_idle_task_init(void)
{
    ...
    ret = os_task_init(&gs_idle_task,
                       ...
                       _k_idle_task_entry,
                       OS_NULL,
                       OS_TASK_STACK_BEGIN_ADDR(idle_stack),
                       OS_TASK_STACK_SIZE(idle_stack),
    ...
    /* Startup */
    ret = os_task_startup(&gs_idle_task);
    if (OS_EOK != ret)
    {
