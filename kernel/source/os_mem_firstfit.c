...
static void _k_firstfit_mem_free(struct heap_mem *h_mem, void *mem)
{
    ...
    if (mem)
    {
        OS_ASSERT_EX((mem >= h_mem->header) && (((os_uint32_t)mem - (os_uint32_t)h_mem->header) <= h_mem->mem_total),
...
static void *_k_firstfit_mem_aligned_alloc(struct heap_mem *h_mem, os_size_t align, os_size_t bytes)
{
    ...
    h = h_mem->header;
    OS_ASSERT_EX((align & (align - 1)) == 0, "unexpected align: %d (should be the power of 2)", align);
...
static os_err_t _k_firstfit_mem_check(struct heap_mem *h_mem)
{
    ...
    while (chunksize > 0)
    {
        ...
        if (rc_lc != c)
        {
            ...
            os_kprintf("mem_check err:chunk:%d, r_chunk:%d, r_chunk's left:%d\r\n", c, rc, rc_lc);
    ...
    if (OS_EOK == ret)
    {
        ...
        {
            os_kprintf("mem_check err:size_total:%d, mem_total:%d, size_used:%d mem_used:%d\r\n",
                    chunksize_total * CHUNK_UNIT, h_mem->mem_total, chunksize_used * CHUNK_UNIT, h_mem->mem_used);
            ret = OS_ERROR;
        }
        else
        {
...
static os_err_t _k_firstfit_mem_trace(struct heap_mem *h_mem)
{
    ...
    while(chunksize > 0)
    {
        ...
        {
            os_kprintf("mem_trace err:chunk:%d, right_chunk:%d, left of right_chunk:%d\r\n", c, _k_right_chunk(h, c), _k_left_chunk(h, _k_right_chunk(h, c)));
            ret = OS_ERROR;
            break;
        }
