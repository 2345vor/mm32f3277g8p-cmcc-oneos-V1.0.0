...
static int delete_directory(const char * dirname)
{
    ...
    if (AMS_NULL == dir)
    {
        ams_err("Open dir: %s failed[%d].", tmp, ret);
        return AMS_ERROR;
    }
...
static int start_download_socket(void)
{
    ...
    ams_strncpy(&payload[4] , &ams.appurl[idx2] , ams_strlen(ams.appurl) - idx2);
    ams_strcat(payload , "\n");
...
static int  parse_message(void)
{
    switch(sm_compute_hash(&(ams._body[0]), 10))
    {
        ...
        PARSER_MSG_PATH(RLISTAPP_HASH, COMMAND_LIST_APP,        EMPTY_FUNCTION);
        PARSER_MSG_PATH(RSTOPAPP_HASH, COMMAND_STOP_APP,        EMPTY_FUNCTION);
        PARSER_MSG_PATH(RUNNGAPP_HASH, COMMAND_RUNNING_APP,     EMPTY_FUNCTION);
        PARSER_MSG_PATH(RESETJVM_HASH, COMMAND_RESET_JVM,       EMPTY_FUNCTION);
        PARSER_MSG_PATH(CHECKCON_HASH, COMMAND_HEARTBEAT,       EMPTY_FUNCTION);
