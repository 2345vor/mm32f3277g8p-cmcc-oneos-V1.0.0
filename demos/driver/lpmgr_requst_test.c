...
static void wakeup_app_entry(void *parameter)
{
    ...
    while (1)
    {
        if (os_event_recv(sleep_info->wakeup_event, WAKEUP_EVENT_BUTTON,
                          ...
                          OS_WAIT_FOREVER, OS_NULL) == OS_EOK)
        {
...
static int wakeup_iqr(int argc, char *argv[])
{
    ...
    sleep_info.wakeup_event = os_event_create("wakeup_iqr");
...
/**
 设备回调函数注册
***/
os_err_t dev_suspend(struct os_device *device, os_uint8_t mode)
{
...
void dev_resume(struct os_device *device, os_uint8_t mode)
{
...
void lp_register_notify(void)
{
    ...
    os_lpmgr_notify_set(user_notify, &data);
