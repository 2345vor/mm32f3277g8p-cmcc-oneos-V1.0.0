...
static os_err_t at45dbxx_init(const char *flash_device_name)
{
    ...
    {
        LOG_E(DBG_TAG,"spi device %s not found!\r\n", AT45DBXX_BUS_NAME);
        return OS_ENOSYS;
    }
    ...
    /* read JEDEC ID */
    {
        ...
        {
            LOG_E(DBG_TAG,"Manufacturer ID or Memory Type error!\r\n");
            LOG_E(DBG_TAG,"JEDEC Read-ID Data : %02X %02X %02X\r\n", id_recv[0], id_recv[1], id_recv[2]);
            return OS_ENOSYS;
        }
        ...
        {
            LOG_E(DBG_TAG,"Memory Capacity error!\r\n");
            return OS_ENOSYS;
        }
    }
	...
	{
		LOG_E(DBG_TAG,"os_calloc failed!\r\n");
		return OS_ENOMEM;
	}
...
static int at45dbxx_read(fal_flash_t *flash, os_uint32_t page_addr, os_uint8_t *buff, os_uint32_t page_nr)
{
    ...
    os_device_read_nonblock(at45dbxx_dev, page, buff, sector_num);
...
static int at45dbxx_write(fal_flash_t *flash, os_uint32_t page_addr, const os_uint8_t *buff, os_uint32_t page_nr)
{
    ...
    for (intex = 0; intex < sector_num; intex++)
    {
        ...
        os_device_write_nonblock(at45dbxx_dev, page + intex, write_buffer, 1);
    }
...
int sfud_flash0_probe(void)
{
    ...
    fal_flash->capacity = MCU_SPI_FLASH_SIZE;
    fal_flash->block_size = MCU_SPI_FLASH_SECTOR_SIZE;
    fal_flash->page_size  = MCU_SPI_FLASH_SECTOR_SIZE;
    
    fal_flash->ops.read_page   = at45dbxx_read;
    fal_flash->ops.write_page  = at45dbxx_write;
    fal_flash->ops.erase_block = at45dbxx_erase;
...
OS_DEVICE_INIT(sfud_flash0_init, OS_INIT_SUBLEVEL_HIGH);
