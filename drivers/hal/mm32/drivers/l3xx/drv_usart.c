...
#define MM32_UART_POLL_TIMEOUT_MS_PER_BYTE (100)
...
static UART_TypeDef *console_uart;

void __os_hw_console_output(const char *str)
{
    ...
    while (*str)
    {
        UART_SendData(console_uart, (uint8_t)*str);
        str++;
    }
}

static int mm32_usart_early_probe(const os_driver_info_t *drv, const os_device_info_t *dev)
{
    console_uart = (UART_TypeDef *)dev->info;
    return 0;
}

OS_DRIVER_INFO mm32_usart_early_driver = {
    .name   = "UART_HandleTypeDef",
    .probe  = mm32_usart_early_probe,
};
OS_DRIVER_DEFINE(mm32_usart_early_driver, CORE, OS_INIT_SUBLEVEL_MIDDLE);
