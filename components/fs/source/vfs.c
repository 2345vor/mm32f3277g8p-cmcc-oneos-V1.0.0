...
int do_openfile(struct vfs_file *fp, const char *path, int flags)
{
    ...
    const char *rel_mnt_path = OS_NULL;
    int ret = 0;
        ...
        if (mnt_point)
        {
            if ((mnt_point->ops) && (mnt_point->ops->open))
            {
                ...
                if (rel_mnt_path)
                {
                    ...
                    fp->flags = flags;
                    if (strcmp(mnt_point->mnt_path, DEVFS_PATH) == 0)
                    {
                        fp->type = FT_DEVICE;
                    }
                    else
...
int do_readfile(struct vfs_file *fp, void *buf, size_t len)
{
    ...
    if (fp && buf && (fp->type != FT_DIRECTORY))
    {
...
{
    int ret = 0;
    struct vfs_mountpoint *old_mnt_point = OS_NULL;
    struct vfs_mountpoint *new_mnt_point = OS_NULL;
    const char *oldpath_rel_mnt = OS_NULL;
    const char *newpath_rel_mnt = OS_NULL;
