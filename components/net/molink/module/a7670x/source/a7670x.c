...
{
    mo_a7670x_t *module = (mo_a7670x_t *)malloc(sizeof(mo_a7670x_t));
    if (OS_NULL == module)
    {
...
int a7670x_auto_create(void)
{
    ...
    if (OS_NULL == device)
    {
        LOG_EXT_E("Auto create failed, Can not find %s interface device!", A7670X_DEVICE_NAME);
        return OS_ERROR;
    }
    ...
    if (OS_NULL == module)
    {
        LOG_EXT_E("Auto create failed, Can not create %s module object!", A7670X_NAME);
        return OS_ERROR;
    }

    LOG_EXT_I("Auto create %s module object success!", A7670X_NAME);

    return OS_EOK;
}
OS_CMPOENT_INIT(a7670x_auto_create);
