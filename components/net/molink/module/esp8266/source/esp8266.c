...
mo_object_t *module_esp8266_create(const char *name, void *parser_config)
{
    ...
    // 20210811 Teternal add for repaire <E/at.parser: Read response buffer failed. The Response buffer size is out of buffer size(n)!>
    if(os_tick_get()<OS_TICK_PER_SECOND*3)
        os_task_msleep((OS_TICK_PER_SECOND*3-os_tick_get())*(1000/OS_TICK_PER_SECOND));
