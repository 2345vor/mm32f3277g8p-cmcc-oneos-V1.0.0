...
static os_err_t sim7070x_set_app_network_pdpidx0(mo_object_t *self)
{
    ...
    if (result != OS_EOK)
    {
        LOG_EXT_E("Module %s set cfun to 0 failed", self->name);
        return OS_ERROR;
    }
    ...
    if (result != OS_EOK)
    {
        LOG_EXT_E("Module %s APP network PDP index0 config to ctnb failed", self->name);
        return OS_ERROR;
    }
    ...
    if (result != OS_EOK)
    {
        LOG_EXT_E("Module %s set cfun to 1 failed", self->name);
        return OS_ERROR;
    }
    ...
    if (result != OS_EOK)
    {
        LOG_EXT_E("Module %s APP network PDP index0 active failed", self->name);
        return OS_ERROR;
    }
    /* for ex: OK\r\n\r\n+APP PDP: 0,ACTIVE\r\n */
    if ((at_resp_get_data_by_kw(&resp, "+APP PDP: 0", "+APP PDP: 0,%s", temp) <= 0) && (strcmp(temp, "ACTIVE") != 0))
    {
        LOG_EXT_E("Module %s APP network PDP index0 active failed, parse resp failed", self->name);
        return OS_ERROR;
    }
    ...
    for (int i = 0; i < SIM7070X_RETRY_TIMES; i++)
    {
        ...
        if (at_resp_get_data_by_line(&resp, 2, "+CGACT: %u,%u", &cid, &stat) <= 0)
        {
    ...
    {
        LOG_EXT_E("Module %s PDP context is deactivate, please activate first", self->name);
        return OS_ERROR;
    }
...
static os_err_t sim7070x_get_app_network_pdpidx0_status(mo_object_t *self, os_uint32_t *stat)
{
    ...
    if (result != OS_EOK)
    {
        LOG_EXT_E("Module %s get APP network PDP index0 status failed", self->name);
        return OS_ERROR;
    }

    /* for ex: +CNACT: 0,1,"100.122.236.108"\r\n +CNACT: 1,0,"0.0.0.0"\r\n */
    if (at_resp_get_data_by_line(&resp, 2, "+CNACT: 0,%u,\"%[^\"]", stat, ipaddr) <= 0)
    {
        LOG_EXT_E("Module %s get APP network PDP index0 status failed, parse resp failed ", self->name);
        return OS_ERROR;
    }

    LOG_EXT_D("Module %s APP network PDP index0 IP addr: %s", self->name, ipaddr);
...
{
    mo_sim7070x_t *module = (mo_sim7070x_t *)malloc(sizeof(mo_sim7070x_t));
    if (OS_NULL == module)
    {
...
int sim7070x_auto_create(void)
{
    ...
    if (OS_NULL == device)
    {
        LOG_EXT_E("Auto create failed, Can not find %s interface device!", SIM7070X_DEVICE_NAME);
        return OS_ERROR;
    }
    ...
    if (OS_NULL == module)
    {
        LOG_EXT_E("Auto create failed, Can not create %s module object!", SIM7070X_NAME);
        return OS_ERROR;
    }

    LOG_EXT_I("Auto create %s module object success!", SIM7070X_NAME);

    return OS_EOK;
}
OS_CMPOENT_INIT(sim7070x_auto_create);
