...
#define DBG_EXT_TAG "sim7070x.ping"
...
os_err_t sim7070x_ping(mo_object_t *self, const char *host, os_uint16_t len, os_uint32_t timeout, struct ping_resp *resp)
{
    ...
    {
        LOG_EXT_E("Ping: AT cmd exec fail: AT+SNPDPID=0");
        result = OS_ERROR;
        goto __exit;
    }
    ...
    {
        LOG_EXT_E("Ping: AT cmd exec fail: AT+SNPING4=\"%s\",1,%u,%u", host, len, timeout);
        result = OS_ERROR;
        goto __exit;
    }
    ...
    {
        LOG_EXT_D("Ping timeout, check network status and please try to ping again");
        result = OS_ETIMEOUT;
    }
    else
    {
        LOG_EXT_D("%s ping: resp parse ip[%s], req_time[%u]", self->name, ipaddr, req_time);
