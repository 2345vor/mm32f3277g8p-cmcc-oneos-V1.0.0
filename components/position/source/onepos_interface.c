...
#define ONEPOS_LOG_TAG "onepos.interface"
...
static onepos_pos_t g_sys_onepos_pos = {0,};

static void onepos_lock_func(void)
{
    if (ONEPOS_RUNING == onepos_get_server_sta())
        os_sem_wait(onepos_lock, OS_WAIT_FOREVER);
}
...
os_bool_t onepos_set_pos_interval(os_int32_t interval)
{
    ...
    if (ONEPOS_MIN_INTERVAL <= interval && ONEPOS_MAX_INTERVAL >= interval)
    {
        ...
        {
            ONEPOS_LOG_I("The interval is already is %ds", onepos_interval);
            return OS_TRUE;
        }
        ...
        if (ONEPOS_RUNING == onepos_get_server_sta())
        {
            ...
            os_timer_set_timeout_ticks(onepos_timer, ticks);
...
static os_bool_t copy_position(onepos_pos_t *dst_info, onepos_pos_t *src_info)
{
    ...
    {
        ONEPOS_LOG_E("wifi_src_info is NULL");
    }
...
static void onepos_circ_run_task_func(void *parameter)
{
    ...
    if (!onepos_init())
    {
        ONEPOS_LOG_I("onepos init failed, will exit serv");
    ...
    while (1)
    {
        ...
        if (ONEPOS_WILL_CLOSE == onepos_get_server_sta())
        {
            onepos_set_server_sta(ONEPOS_CLOSING);
            onepos_exit();
            ONEPOS_LOG_D("exit serv succ");
        }

        memset(&curr_pos_info, 0, sizeof(onepos_pos_t));

        os_sem_wait(onepos_sem, OS_WAIT_FOREVER);
        os_timer_start(onepos_timer);
        ...
        if (onepos_get_dev_sta())
        {
            if (!onepos_mqtt_is_connected())
            {
                ...
                {
                    ONEPOS_LOG_I("onepos mqtt re_connect error, will delay %d ms\n", ONEPOS_WAIT_MQTT_READY);
...
static void onepos_circ_run_init(void)
{
    ...
    onepos_sem = os_sem_create("onepos_sem", 0, 1);
    if (onepos_sem == OS_NULL)
    {
        ONEPOS_LOG_D("create semaphore failed.\n");
        return;
    }

    onepos_lock = os_sem_create("onepos_lock", 1, 1);
    if (onepos_lock == OS_NULL)
    {
        ONEPOS_LOG_D("creat semaphore failed.\n");
        return;
    }
    ...
    {
        ONEPOS_LOG_D("create timer failed.\n");
        return;
    }
...
static void onepos_single_run(void)
{
    ...
    if (!onepos_init())
    {
        ONEPOS_LOG_I("onepos init failed, will return");
        goto exit_sevr;
    }
    ...
    if (onepos_get_dev_sta())
    {
        ...
        {
            ONEPOS_LOG_E("onepos mqtt is not connect!");
        }
    }
    else
    {
